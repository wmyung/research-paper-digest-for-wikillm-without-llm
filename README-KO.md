...
결정론적 컴파일러 자체는 계속 모델을 사용하지 않습니다. 남은 실패에는 저장소의
[`validated-cascade-workflow`](skills/validated-cascade-workflow/SKILL.md)
스킬을 적용할 수 있습니다. 프로그램 통과본은 보존하고, 결정론적 수리를 먼저
수행하며, 그래도 남은 적격 실패에만 설정된 언어모델을 사용합니다. 마지막에는
동일한 fail-closed 검증과 저장 결과 readback을 요구합니다. 특정 모델이나 논문
형식은 범용 스킬에 고정하지 않고 adapter로 분리합니다.

Glossary의 `Term 1` 같은 번호형 자리표시자는 이제 hard failure입니다. 단순히
글머리표 수와 길이를 채운 추출 문장은 의미 있는 용어집으로 통과하지 않습니다.
...
| 정체된 레코드 | 2단계 수리: 실패한 게이트마다 표적 연산자를 적용하고 단조 수용 규칙으로 판정 |
...
- 누락된 근거를 만들어내지 않는 최대 4회의 자동 보완(1단계)
- 실패한 게이트를 읽고 게이트별 표적 수리를 수행하는 2단계 진단 기반 보완
- 모든 hard gate와 0.95 이상 점수를 통과할 때만 `SOURCE_READY`

`quality_score`는 hard error가 하나라도 있으면 `임계값 - 0.01`로 고정되므로
실패한 레코드는 모두 같은 값으로 보고됩니다. 실패 레코드의 우선순위는 고정되지
않은 `raw_quality_score`로 판단해야 합니다.

1단계는 QA 보고서를 보지 않고 근거 예산만 키워 다시 컴파일하며, 문장을 먼저
차지한 타깃이 그것을 계속 보유합니다. 그래서 구조적으로 실패한 레코드는 4회를
돌려도 그대로입니다. 2단계는 실패한 게이트를 읽고 게이트마다 대응하는 수리
연산자를 *선택 집합*에 적용한 뒤, 그 선택으로부터 본문·근거 원장·커버리지
원장·검색 질문을 전부 다시 생성합니다. 산문이 아니라 선택을 고치기 때문에 모든
문장이 여전히 원문의 축자 구간이고, 수리가 grounding 게이트를 깨뜨릴 수
없습니다. 이미 통과하던 검사를 실패시키는 제안은 폐기하고,
`(오류 감소, raw 점수 상승, 경고 감소)` 순서로 엄격히 개선될 때만 채택합니다.
채택·기각 이력은 `qa.stage2`에 남습니다. 원문이 채울 수 없거나 유사중복
문장으로만 채울 수 있는 부족분은 고치지 않고 `NOT_SOURCE_READY`로 남깁니다.
