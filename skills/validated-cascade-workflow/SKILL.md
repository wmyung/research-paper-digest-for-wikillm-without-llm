...
8. Parse model output as an untrusted proposal. Rebuild evidence and provenance deterministically, then run the same validator used for program output. Retry the cheaper model stage once when evidence shows a repairable validation failure. At its generation limit, require a structured gap assessment before either expensive fallback or terminal settlement; missing assessment stays queued. Admit the expensive source only when the assessment proves that the allowlisted input cannot supply the missing evidence.
